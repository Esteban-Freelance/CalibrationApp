...
namespace CalibrationApp.Devices.Real
{
    public class Keithley3706A : IMeasurementDevice
    {
        private readonly Keithley3706Config _config;
        ...
        public Keithley3706A(string id, string name, Keithley3706Config config)
        {
            Id = id;
            Name = name;
            _config = config;
        ...
        public async Task<MeasurementResult> MeasureAsync(MeasurementParameters parameters)
        {
            ...
            try
            {
                ...
                // 2. Enable auto range (unless a specific range was set beforehand)
                await Send("dmm.autorange = dmm.ON");

                // 3. Set auto delay for settling
                ...
                if (parameters.SampleCount <= 1)
                {
                    ...
                    value = ParseDouble(response);
                }
                else
                {
                    // Multiple readings — average them
                    int count = parameters.SampleCount;
                    await Send($"dmm.measurecount = {count}");

                    // Create buffer, measure, compute average
                    await Send($"buf = dmm.makebuffer({count})");
                    await Send("dmm.measure(buf)");
                    string response = await Query($"print(buf.mean)");
                    value = ParseDouble(response);
                ...
                // Optional settling delay between measurements
        ...
        {
            if (type is not (MeasurementType.VoltageDC or MeasurementType.CurrentDC))
...
public class Keithley3706Config
{
